...
const Duplicated = ({navigation,route}) =>{
    const [success, setSuccess] = useState(false);
    const [filter,setFilter]=useState(userStore.sameFilterType)
    ...
    useEffect(()=>{
        AsyncStorage.getItem('dupFirst').then(data=>{
            if(!data){
                setTimeout(()=>{

                navigation.navigate('Divider',{page:'duplicated'})
                AsyncStorage.setItem('dupFirst',JSON.stringify(true))
                },250)
            }
        })
                    ...
                    if(indis == 0){
                        return(
                            <TouchableWithoutFeedback  onPress={()=>{
                                ...
                                userStore.setSameFilter(item.type)
                                }
                            }}>
    ...
    let renderImages= ({item, index,group,indis})=>{
        ...
        if(index==0){
            ...
            return(
                <TouchableWithoutFeedback onPress={()=>navigation.navigate('Detail',{group,index,page:'same'})}>
                    <View   style={styles.twiceContainers}>
                        ...
                        } source={{uri:uri,priority:FastImage.priority.high,cache:'force-cache'}}   style={{width:'100%',height:'100%',borderRadius:20}}/>
                        ...
                        <View style={{position:'absolute',width:'50%',left:10,top:-15,zIndex:10}}>
                            <TouchableOpacity style={{paddingVertical:5,borderRadius:20,backgroundColor:'#9BF538'}} onPress={()=>{
                                setSuccess(true)
                                userStore.keepItArraySame(group.images,indis)
                            }}>
                        ...
                        <View style={{position:'absolute',right:10,top:10,width:30,height:30,zIndex:99}}>
                            <TouchableOpacity onPress={()=>{
                                ...
                                <FastImage source={userStore.sameSelected.indexOf(item) == -1 ? require('../../../assets/tick_close.png'):require('../../../assets/tick_open.png')} style={{width:30,height:30,zIndex:-1}}/>
                            </TouchableOpacity>
                        </View>
                ...
                <TouchableWithoutFeedback onPress={()=>navigation.navigate('Detail',{group,index})}>
                    <View  style={styles.twiceContainers}>
                        ...
                        <TouchableOpacity onPress={()=>{
                            ...
                            <FastImage source={userStore.sameSelected.indexOf(item) == -1 ? require('../../../assets/tick_close.png'):require('../../../assets/tick_open.png')} style={{width:30,height:30}}/>
                        </TouchableOpacity>
                    </View>
                </TouchableWithoutFeedback>
            )
    ...
    let renderItem= ({item, index})=>{
        let allSelected=item.images.find((it,indis)=>{
            if(userStore.sameSelected.indexOf(it) == -1) return false
                ...
                <View style={styles.menuItemContainer}>
                    <View style={styles.menuInsideContainer}>
                        ...
                        <View style={styles.menuLeftContainer}>
                            ...
                            <Text style={[styles.menuText,{color:userStore.activeTheme.textColor}]}>{I18n.t('sameLine',{total:item.images.length})}</Text>
                        ...
                        <TouchableOpacity style={styles.menuRightContainer} onPress={()=>{
                            if(allSelected){
                                item.images.find(it=>{
                                    ...
                                    // }
                                    if(userStore.sameSelected.indexOf(it)!=-1)
                                        userStore.setSameSelected(it)
                    ...
                    <View style={{flexDirection:'row',flex:1}}>
                        ...
                        <FlashList data={item.images.slice()} estimatedItemSize={item.images.length}

                                   showsHorizontalScrollIndicator={false} horizontal={true} renderItem={(info)=>  renderImages({item:info.item,index:info.index,group:item,indis:index}) } />
        ...
        <View style={[styles.container,{backgroundColor:userStore.activeTheme.background}]}>
                ...
                <TouchableOpacity style={{width:60,height:40,justifyContent:'center'}} onPress={()=>navigation.goBack()}>
                    ...
                    <TouchableOpacity onPress={()=>{
                        if(!userStore.premium){
                            ...
                            if(userStore.selectedAllSame){
                                userStore.deselectAllSame()
                            }else{
                                userStore.selectAllSame()

                            }
                        }

                        // userStore.same.find(it=>{
                        //     it.images.find((item,indis)=>{
                        //
                        //         if(it.images[0] != item) userStore.setSameSelected(item)
                        //     })
                        // })
                    }} style={[styles.buttonsContainer,{backgroundColor:userStore.activeTheme.homeBoxBackground}]}>
                        <FastImage source={userStore.selectedAllSame ? require('../../../assets/tick_open.png'):require('../../../assets/tick_close.png')} style={{resizeMode:'contain',width:23,height:23}}/>
                        <Text style={{fontFamily:'Roboto-Medium',color:userStore.activeTheme.textColor,marginLeft:10}}>{userStore.selectedAllSame ? I18n.t('selectedAll'):I18n.t('selectAll')}</Text>
                    </TouchableOpacity>

            </View>
            <View style={styles.headerContainer}>
                <Text style={[styles.homeTitle,{color:userStore.activeTheme.textColor}]}>{I18n.t('samePhotos')}</Text>

                <View style={{}}>
                    <Text style={[styles.headerInfoText,{color:userStore.activeTheme.textColor}]}>{I18n.t('similarsTotal',{total: userStore.sameTotal} )}</Text>
                    <Text style={[styles.headerInfoText, {textAlign: 'right',color:userStore.activeTheme.textColor}]}>{I18n.t('similarSelected',{total: userStore.sameSelected.length} )}</Text>
            ...
            {userStore.same.length>0?(
                <View style={styles.mainContainer}>
                    ...
                    <FlashList data={userStore.same.slice()} renderItem={renderItem}   />
            ...
            {userStore.sameSelected.length>0 && (
                <View style={{width:'100%',position:'absolute',justifyContent:'center',alignItems:'center',bottom:50}}>
                    <TouchableWithoutFeedback style={{

                    }}
                                              onPress={()=>{
                                                  if(!userStore.premium && userStore.sameSelected.length>1){
                                                      navigation.navigate('Premium')
                                                  }else{
                                                      if(!userStore.premium){
                                                          navigation.navigate('Premium')
                                                          userStore.setDeletePage('same')
                    ...
                    >
                        <View style={{paddingHorizontal:20,height:60,backgroundColor:!userStore.premium && userStore.sameSelected.length>1 ? '#959594': '#0011FA',justifyContent:'center',alignItems:'center',borderRadius:33}}>
                        {userStore.deleteLoading ? (
                                <Spinner size={'large'} color={'white'}/>
                                ):(
                                <Text style={{color:'white',fontFamily:'Roboto-Medium',fontSize:20}}>
                                    {I18n.t('deleteSame',{total:userStore.sameSelected.length})} ({userStore.formatBytes(userStore.sameTotalSize)})
...
export default observer(Duplicated)
